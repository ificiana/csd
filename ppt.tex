...
\author{
    ...
    \vspace{0.5cm}
    \small \textbf{Term Project for ME5780 Control Systems Design} \\
    \vspace{0.5cm}
    \normalsize
...
% Blocks
\node[draw, rectangle, minimum width=2.5cm, minimum height=1cm] (traj) at (0,0) {Trajectory\\Generator};
\node[draw, rectangle, minimum width=2.5cm, minimum height=1cm] (pid) at (4,0) {PID Position\\Controller};
\node[draw, rectangle, minimum width=2.5cm, minimum height=1cm] (geom) at (8,0) {Geometric\\Attitude Control};
\node[draw, rectangle, minimum width=2.5cm, minimum height=1cm] (alloc) at (8,-2.5) {Thrust\\Allocation};
\node[draw, rectangle, minimum width=2.5cm, minimum height=1cm] (plant) at (4,-2.5) {Payload +\\Drones};

% Arrows
\draw[->, thick] (traj) -- node[above] {$\mathbf{x}_d, \mathbf{v}_d, \mathbf{a}_d$} (pid);
\draw[->, thick] (pid) -- node[above] {$\mathbf{F}_d$} (geom);
\draw[->, thick] (geom) -- node[right] {$\boldsymbol{\tau}_d$} (alloc);
\draw[->, thick] (alloc) -- node[below] {$\{\mathbf{T}_i\}$} (plant);
\draw[->, thick] (plant) -- ++(-2,0) |- node[left, pos=0.75] {$\mathbf{x}, \mathbf{v}, \mathbf{R}, \boldsymbol{\omega}$} (pid);
\draw[->, thick] (plant) -- ++(2,0) |- (geom);
\end{tikzpicture}
\end{center}

\textbf{Hierarchical Structure:}
\begin{itemize}
    \item Outer loop: Position control (PID)
    \item Inner loop: Attitude control (Geometric on SO(3))
\end{itemize}
\end{frame}
